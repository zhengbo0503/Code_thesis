...
%GET_REFERENCE Compute reference singular/eigen-vectors using multiprecision arithmetic.
...
%   71-digit precision via Advanpix mp.
%
%   Use with compute_error_svecs(..., Vmp, S) to avoid recomputing the reference
%   when comparing multiple algorithms on the same matrix.

% Set precision on the reference object without changing solver defaults.
Amp = mp(A, 71);
