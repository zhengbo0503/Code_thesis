...
fprintf("[START] svecs_left_varying_cond\n");
...
for i = 1:2
    ...
    for j = 1:length(kkappa)
        ...
        % Our algorithm
        [U1,S1,V1,nos1,scnd] = mposj_svec(A);
        [Uref, Sref] = get_reference(A, 'l');
        [err1, relgap1] = compute_error_svecs(A, U1, 'l', Uref, Sref);
        ...
        [err2, ~ ] = compute_error_svecs(A, U2, 'l', Uref, Sref);
        ...
        [err3, ~ ] = compute_error_svecs(A, U3, 'l', Uref, Sref);
        err_dgejsv(j,i) = max(err3);

        % MATLAB
        [U4,S4,V4] = svd(A,'econ');
        [err4, ~] = compute_error_svecs(A, U4, 'l', Uref, Sref);
...
savedata = 0;
if savedata
    save("../data/svecs_left_varying_cond.mat");
end

writecsv = 1;
if writecsv
    plotdata = table(kkappa(:), ...
        err_mposj(:,1), err_dgesvj(:,1), err_dgejsv(:,1), err_matlab(:,1), bound2(:,1), ...
        err_mposj(:,2), err_dgesvj(:,2), err_dgejsv(:,2), err_matlab(:,2), bound2(:,2), ...
        err_mposj(:,3), err_dgesvj(:,3), err_dgejsv(:,3), err_matlab(:,3), bound2(:,3), ...
        'VariableNames', {'kappa', ...
        'mp3_m3', 'dgesvj_m3', 'dgejsv_m3', 'matlab_m3', 'bound_m3', ...
        'mp3_m4', 'dgesvj_m4', 'dgejsv_m4', 'matlab_m4', 'bound_m4', ...
        'mp3_m5', 'dgesvj_m5', 'dgejsv_m5', 'matlab_m5', 'bound_m5'});
    write_figure_csv("../data/svecs_left_varying_cond.csv", plotdata);
end
...
if drawgraph
    ...
    for mode = 1:3
        ...
        hold on;
        loglog(kkappa,err_dgesvj(:,mode),'LineStyle','none','Marker','pentagram', 'Color', C2);
        loglog(kkappa,err_dgejsv(:,mode),'LineStyle','none','Marker','square', 'Color', C3);
...
fprintf("[END] svecs_left_varying_cond\n");
